...
class Student {
	...
	Student(String name, int stuID, int age, String stuAdd, String stuPhone, double weight) {
		...
		if (month >= 1 && month <= 12) {
			...
			System.out.println("잘못된 입력값입니다. 1 ~ 12 사이의 정수값을 할당하세요.");
			return; 		//프로그램을 종료
		}
}
	...
	//toStirng() 재정의:객체의 Heap 메모리의 주소값 출력, hashcode로 출력(16진수)
	//객체 자체를 출력했을 시 toStirng() 가 작동
	@Override
...
public class Initial_Field01 {
	...
	public static void main(String[] args) {
		...
		//	getter(필드의 값을 읽을 때 ), setter(필드의 값을 할당할 때)를 사용해서 값을 할당 			
		...
		s1.stuAdd="서울 종로구";
		s1.stuID=1111;
		s1.stuPhone = "010-5048-5551";
		s1.weight=80.5;
		s1.month=50;
		
		System.out.println(s1);				//객체를 출력했을 때 toStirng() 메소드가 호출
		System.out.println(s1.toString());
		System.out.println("=================");
		
		//2. 객체를 생성시 생성자를 통해 필드의 값을 할당
		Student s2 = new Student("이순신", 2222, 30, "서울 강남구", "010-5048-5551", 85.5);
		System.out.println(s2);
		System.out.println(s2.toString());
		System.out.println("=============");
		//3. Setter를 사용해서 값을 넣고 getter를 사용해서 필드의 값 출력
