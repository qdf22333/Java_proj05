...
public class EX02 {
	...
	public static void main(String[] args) {
		...
		//1.배열 선언 : - 동일한 데이터 타입을 저장. - 방의 크기가 지정되면 수정이 불가능
		
		int [] arr1;			//선언 
		arr1 = new int[500];	//방크기를 정의
		
		//배열에 저장된 기본 값을 출력 : Heap 강제 초기값이 할당됨. 0
		System.out.println("배열의 초기값 출력");
		for (int i=0; i<arr1.length; i++) {		//arr1.length : 배열의 방의 갯수
			System.out.print(arr1[i]+ " ");		//0 ~ 499번 방 까지 출력
		}
		System.out.println();
		...
		//0번 방 부터 마지막 방 까지 순환하면서 출력, for문 보다는 구문이 짧음
		for (int d: arr1) {
			System.out.print(d+ " ");
		}
