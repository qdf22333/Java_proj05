...
public class EX03 {
	...
	public static void main(String[] args) {
		...
		System.out.println(arr1.length);	//5, arr1.length : 행의갯수
		...
		for (int i = 0; i<arr1.length; i++) {
			//열의 정의 : j<== 열의 방번호, arr[i].length <== i 행의 열의 갯수
			for (int j=0;j<arr1[1].length; j++, a++) {
				arr1[i][j]= a+1;
			}
		}
		...
		//4.  Enhanced for문을 사용해서 값 출력
		for (int [] k : arr1) {	//arr1의 루프를 돌면서 행을 꺼내와서 k 변수에 할당
			for (int b:k) {	//일차원 배열의 각 방의 값을 끄집어 와서 b변수에 담는다.
				System.out.print(b + " ");
			}
			System.out.println(); 
