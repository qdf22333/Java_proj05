...
interface A {	//인터페이스 : 추상메소드만 존재해야함 (예외: defatult, static)
	default void abc1() {
...
class C {	//추상클래스는 : 추상메소드 + 일반메소드 + 일반필드
	C () {}
	void cde() {
...
//class D는 C 클래스를 상속하고 있고, A, B 인터페이스를 구현하고 있다.
	...
	//extends : 하나의 부모만 올 수 있다. (다중상속 불가능)
	//implements : 여러 부모를 가질 수 있다. 인터페이스 : 다중 상속 가능
