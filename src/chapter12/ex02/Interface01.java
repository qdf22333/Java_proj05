...
//정식 표현
interface A {	//인터페이스 이름 : 첫자는 반드시 영문 대문자.
	//모든 필드는 : public static final : 값을 수정 불가
		//static : 객체생성없이 호출 가능. A.a 
...
interface B {
	...
	//public abstract 생략 : 모든 메소드는 선언부만 있고 구현부가 없어야 한다. (예외, default, static)
	void abc();
}
...
//추상 클래스 : 추상 메소드를 포함한 클래스, 추상 클래스 내의 추상메소드는 abstract키를 생략하면 안됨.
abstract class C {
	
	int m = 4;		//일반 인스턴스 필드 : 수정이 가능
	
	void bcd () {	//일반 메소드 : 구현부가 있는 인스턴스 메소드
		System.out.println("추상 메소드 C 클래스의 메소드");
		
	}
	//추상 클래스 내의 추상 메소드는 abstract 키를 붙여줘야함 
	abstract void abc ();
	...
	public static void main(String[] args) {
		...
		인터페이스 (Interface) : 내부와 외부를 연결시켜주는 접점, 콘센트 
			...
			- 모든 필드는 public static final이 생략됨
				: 값을 수정할 수 없다.
			- 모든 메소드는 public abstract 이 생략됨 (예외, default, static 메소드는 예외 : 1.8 버전에서 추가)
				: 모든 메소드는 선언부만 있어야 한다. 구현부가 있으면 안됨 : {}
				: 예외, default, static 메소드는 인터페이스에서 유일하게 구현부를 가지는 메소드
			- 하나의 클래스는 하나의 부모클래스를 상속해야한다., 자바는 다중 상속 불가.	
			- 하나의 클래스는 여러개의 인터페이스를 다중 상속 가능.
		 */
...
//		System.out.println(C.m);	//인스턴스 필드 : 객체 생성 후 호출 불가능
		
		c2.m = 44;
		System.out.println(c2.m);	//44 : 값 수정 가능
		
		c2.bcd();	//C 클래스의 일반 메소드 (인스턴스 메소드)
		c2.abc();	//오버라이딩된 메소드 출력, C 타입의 abc ()를 호출, Cc타입의 abc 출력
