...
//하나의 Java 파일에서 Public class는 하나만 올 수 있음


//외부 패키지에서는 접근이 불가
class Product {		//클래스는 : 객체를 생성하기 위한 틀, ==> 객체화를 하기 위한 틀
	...
	//2. 생성자 : 1. 리턴타입이 존재하지 않음, 2. 클래스 이름과 동일해야한다.
			//클래스 이름과 동일한 이름을 가지고 리턴 타입이 없는 메소드
			//객체를 생성시 생성자를 호출. 필드의 기본값을 할당
		//기본생성자: 필드의 기본값을 로드
	Product(){}	//기본 생성자: 인풋 값이 없는 생성자. 생략가능, 
				//다른 생성자가 존재할 경우 반드시 명시해야함 
