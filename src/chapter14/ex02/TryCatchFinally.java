	...
	public static void main(String[] args) {
		...
		*/
		...
		try {
			...
			//try 블락에서 예외가 발생되지 않으면 catch 블락은 실행되지 않음
			System.out.println(3/0);	//실행예외 : (ArithmeticException)
			
		} catch (Exception e) {
			//try 블락에서 ArithmeticException 이 발생 되었을 때 작동
			//try 블락에서 ArithmeticException 이 발생이 되지 않았을 때 는 작동되지 않음
		...
		try {
			...
			//try 블락에서 Exception이 발생 되었을 때 실행
			System.out.println("숫자 0으로는 나눌 수 가 없습니다.");
