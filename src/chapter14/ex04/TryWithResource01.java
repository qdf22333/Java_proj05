...
public class TryWithResource01 {
	...
	public static void main(String[] args)  {
		...
		//1. 리소스 수동 해제 : finally 블락에서 객체를 close() 메소드를 호출해서 
		...
		//일반예외 (Catched Exception) : 프로그램 작성시 컴파일 단계에서 오류
			...
			//2. 예외를 내가 처리 :	try -  catch 블락을 만들어서 내가처리
		
		ir1 = new InputStreamReader(System.in);
		//예외를 내가 처리
		try {
			char input = (char) ir1.read();		//예외처리가 되어야 함. 일반예외 : IOException
			...
			//일반적으로 객체를 사용후 finally 블락에서 객체를 제거함.
				//객체를 제거할 때도 예외처리가 되어야 됨.
			
			if (ir1 != null) {	//객체가 null일 때는 제거하지 않고, 객체가 null이 아닐때 제거함
				try {
